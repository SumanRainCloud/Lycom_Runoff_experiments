...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!AADDEEDD please check
      do l =1,nsoil
        ...
        if (l .eq. 1) then
          ...
          Qin1(i,t,j,l) = rain*(3600)-((2.0/p_LAImax)*(35/100)*rain*3600) !!check if the coordinate system is right
...
!          W_c1(i,t,j,l)=W_c1(i,t,j,l)+Qin1(i,t,j,l)
      ...
      Wx1(i,t,j)= Wx1(i,t,j) + (Q_Oflow1 + Q_Per1 - Q_base1(i,t,j))
...
!COMMENTED        fH2Ol_tb_f1 = p_rmaxH2Ol_g1-fH2Ol_ts1(i,t)
      ...
      Wx1(i,t,j)= Wx1(i,t,j) + (Q_Per1 + Q_Oflow1 - Q_base1(i,t,j))
        
      Overflow= max(0.0, Wx1(i,t,j)-Wxmax)
        
      Wx1(i,t,j) = Wx1(i,t,j) - Overflow
        
      !Bulk soil (bucket) !!!Transpiration from the bucket by other vascular forest!!!
      ...
      S2(j) = max(0.0, min(1.0, Wx1(i,t,j) / Wxmax))  ! Relative water content in the bucket
      if (S2(j) .gt. 1.0) then
        write(*,*) "The Soil saturation level(bucket) is more than 1 at", i
      endif       
...
if (day .eq. dpm .and. ts .eq. tspd) then
  ...
  do j = 1,p_nspec !
    ...
    !M0(j)                          = M0(j) + kMrt0* bsum(j)*dpm*tspd
      ...
      gpp0(i,j) = npp0(i,j) * c_MCo2 * Ngrow * p_dt
